...
class OutputCfg:
    ...
    cat_name: str
    target: str
    # Optional HiPS metadata fields; when not provided, sensible defaults are used.
...
def _now_str() -> str:
    ...
    HiPS 1.4 requires hips_release_date in the form YYYY-mm-ddTHH:MMZ.
    We always use UTC here to avoid timezone ambiguities.
    """
...
def write_properties(out_dir: Path, output_cfg: OutputCfg, level_limit: int, n_src: int, tile_format: str = "tsv"):
    """
    Write the HiPS 'properties' file in compliance with the IVOA HiPS 1.4 specification
    for catalogue HiPS.

    The mapping used here is:
      * creator_did      : unique IVOID for this HiPS, built from the label or taken from the config
      * obs_title        : short human-readable title, defaulting to the catalogue name
      * dataproduct_type : 'catalog' for catalogue HiPS
      * hips_version     : fixed to '1.4' (this implementation targets HiPS 1.4)
      * hips_release_date: current UTC time in ISO 8601 (YYYY-mm-ddTHH:MMZ)
      * hips_status      : 'public master unclonable' (can be changed if needed)
      * hips_tile_format : 'tsv' for catalogue tiles
      * hips_order       : deepest HiPS order (level_limit)
      * hips_frame       : 'equatorial' (ICRS)
    """
    now_iso = _now_str()
    label = output_cfg.cat_name

    # Use values from the config if provided; otherwise fall back to safe defaults.
    ...
    # Backward-compatibility / legacy fields (ignored by the spec but harmless)
